...
with open("MNIST_training_data.txt", "w") as outfile:
    outfile.write("Lines: " + str(Ndatapoints) + " Linelength: " + str(int(28*28)) + "\n")
    for k in range(Ndatapoints):
        for l in range(28*28):
            outfile.write(str(trainX[k].flat[l]/255) + "\n")
...
def test_func_RBM():
    ...
    #Train the RBM
    R = rbm(nvisible=28*28, nhidden=14*14, nCDsteps = 1, nepochs = 15)
